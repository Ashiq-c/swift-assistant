<script>
	...
	import MessagePopup from '$lib/components/common/MessagePopup.svelte';
	import { popupState, hidePopup } from '$lib/stores/popup.js';
	...
	const chatEventHandler = async (event, cb) => {
		...
		if ((event.chat_id !== $chatId && !$temporaryChatEnabled) || isFocused) {
			...
			if (type === 'execute:python') {
				...
				// Pyodide support has been removed to reduce project size
				if (cb) {
					cb({
						stdout: '',
						stderr: 'Python execution is not available. Pyodide support has been removed.',
						result: null
					});
				}
	...
	onMount(async () => {
		...
		// Force theme to default to light (reset any existing dark theme)
		...
		if (backendConfig) {
			...
			if ($config) {
				...
				if ($config.features?.enable_websocket && !backendConfig.name?.includes('Custom API')) {
					await setupSocket($config.features.enable_websocket);
				}

				// Skip authentication for frontend-only mode (Custom API)
				if (backendConfig.auth === false || backendConfig.name?.includes('Custom API')) {
					console.log('🔓 Running in frontend-only mode, skipping authentication');
					// Create a mock user for frontend-only mode
					const mockUser = {
						id: 'frontend-user',
						name: 'Frontend User',
						email: 'user@frontend.local',
						role: 'user',
			...
			// This should not happen now since we provide a mock config
			console.error('No backend config available');
			await goto(`/error`);
		}
...
<MessagePopup
	bind:show={$popupState.show}
	message={$popupState.message}
	type={$popupState.type}
	title={$popupState.title}
	autoClose={$popupState.autoClose}
	autoCloseDelay={$popupState.autoCloseDelay}
	on:close={hidePopup}
/>
