<script lang="ts">
	...
	// atSelectedModel removed - using static model
	export let selectedModels: ['static-model'];
	...
	// Simplified capability checks for static model - assume all capabilities are available
	let visionCapableModels = ['static-model'];
	let fileUploadCapableModels = ['static-model'];
	let webSearchCapableModels = ['static-model'];
	let imageGenerationCapableModels = ['static-model'];
	let codeInterpreterCapableModels = ['static-model'];

	// Simplified for static model
	let toggleFilters = [];
				...
				<div class="w-full relative">
					...
					<Commands
						...
						onSelect={(e) => {
							...
							// Model selection removed - only other command types handled here
			...
			>
				<div class="">
					...
					{:else}
						...
						>
							<div
								class="flex-1 flex flex-col relative w-full rounded-2xl border border-gray-200 hover:border-gray-300 focus-within:border-gray-300 transition px-4 py-3 bg-white"
							...
							>
											...
											{#if file.type === 'image'}
												<div class=" relative group">
													<div class="relative flex items-center">
														...
														/>
														<!-- Vision capability check removed for static model -->
													</div>
								...
								<div class="px-2.5">
									{#if $settings?.richTextInput ?? true}
										<div
											class="scrollbar-hidden rtl:text-right ltr:text-left bg-transparent dark:text-gray-100 outline-none w-full pt-2.5 pb-[5px] px-1 resize-none h-fit max-h-80 overflow-auto"
									...
									{:else}
										<textarea
											...
											dir={$settings?.chatDirection ?? 'auto'}
											bind:this={chatInputElement}
											class="scrollbar-hidden bg-transparent dark:text-gray-200 outline-none w-full pt-3 px-1 resize-none"
											placeholder={placeholder ? placeholder : $i18n.t('Send a Message')}
											bind:value={prompt}
											on:input={() => {
												command = getCommand();
								...
								<div class="flex items-center justify-between px-2 py-2" dir="ltr">
									<div class="flex items-center gap-2">
										...
										>
											<div
												class="text-gray-500 hover:text-gray-700 transition rounded-full p-1.5"
											>
												<svg
													xmlns="http://www.w3.org/2000/svg"
													viewBox="0 0 20 20"
												...
												{#if showWebSearchButton}
													<Tooltip content={$i18n.t('Search the internet')} placement="top">
														<button
															...
															type="button"
															class="px-2 @xl:px-2.5 py-2 flex gap-1.5 items-center text-sm rounded-full transition-colors duration-300 focus:outline-none max-w-full overflow-hidden hover:bg-gray-50 dark:hover:bg-gray-800 {webSearchEnabled ||
												...
												<Tooltip content={$i18n.t('Send message')}>
													<button
														...
														class="{!(prompt === '' && files.length === 0)
															...
															: 'text-white bg-gray-300 disabled'} transition rounded-full p-2 self-center"
														type="submit"
