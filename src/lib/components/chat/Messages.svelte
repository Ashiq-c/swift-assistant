<script lang="ts">
	...
	$: if (history.currentId) {
		console.log('🔍 Messages.svelte - Creating messages from history.currentId:', history.currentId);
		let _messages = [];
		...
		messages = _messages;
		console.log('🔍 Messages.svelte - Created', messages.length, 'messages');
	} else {
		console.log('🔍 Messages.svelte - No history.currentId, setting messages to empty');
		messages = [];
	}
...
<div class={className}>
	{#if Object.keys(history?.messages ?? {}).length == 0 && (!chatHistory || chatHistory.length == 0)}
		<ChatPlaceholder modelIds={selectedModels} {onSelect} />
	{:else}
		<div class="w-full pt-2">
			{#key chatId}
				<div class="w-full">
					{#if messages.at(0)?.parentId !== null && (!chatHistory || chatHistory.length === 0)}
						<Loader
							on:visible={(e) => {
								console.log('visible');
								if (!messagesLoading) {
