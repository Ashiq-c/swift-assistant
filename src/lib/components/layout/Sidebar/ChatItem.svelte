...
>
	<div class=" text-sm text-white/70 flex-1 line-clamp-3">
	...
	{#if confirmEdit}
		<div
			...
			confirmEdit
				...
				: selected
					? 'bg-white/15'
					: 'group-hover:bg-white/10'}  whitespace-nowrap text-ellipsis relative {generating
				? 'cursor-not-allowed'
				: ''}"
		>
			<input
	...
	{:else}
		<a
			...
			confirmEdit
				...
				: selected
					? 'bg-white/15'
					: ' group-hover:bg-white/10'}  whitespace-nowrap text-ellipsis"
			href="/c/{id}"
			on:click={() => {
				dispatch('select');
