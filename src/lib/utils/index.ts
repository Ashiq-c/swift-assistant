...
export const processResponseContent = (content: string) => {
	// First process Chinese content
	content = processChineseContent(content);
	
	// Fix common markdown formatting issues caused by extra spaces
	content = fixMarkdownSpacing(content);
	
	return content.trim();
};

function fixMarkdownSpacing(content: string): string {
	// Fix broken markdown formatting by removing extra spaces
	return content
		// Fix broken bold formatting: ** text ** -> **text**
		.replace(/\*\* ([^*]+) \*\*/g, '**$1**')
		// Fix broken italic formatting: * text * -> *text*
		.replace(/\* ([^*]+) \*/g, '*$1*')
		// Fix broken headings: # text -> #text (but preserve space after #)
		.replace(/^(#{1,6}) +/gm, '$1 ')
		// Fix broken inline code: ` text ` -> `text`
		.replace(/` ([^`]+) `/g, '`$1`')
		// Fix broken links: [ text ]( url ) -> [text](url)
		.replace(/\[ ([^\]]+) \]\( ([^)]+) \)/g, '[$1]($2)')
		// Fix common word breaks like "built -in" -> "built-in"
		.replace(/(\w+) -(\w+)/g, '$1-$2')
		// Fix "Here 's" -> "Here's"
		.replace(/(\w+) '(\w+)/g, "$1'$2");
}
